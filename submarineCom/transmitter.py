...
if __name__ == "__main__":
    ...
    while(True):
        '''
        signal = transmitter.signal(tm.timeInterval[i])
        for j in range(1000):
           #TODO: can only send str type of data
            data = str(signal[j])
            channel.send_signal(data)
        '''
        transmitter.generate_frequency(tm.timeInterval[data[i]])
        generated_signal = transmitter.generate_chirp(tm.timeInterval[data[i]])
